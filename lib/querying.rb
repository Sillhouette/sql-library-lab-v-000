...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM series JOIN characters ON characters.series_id = series.id WHERE characters.species = 'human' HAVING MAX(species)"#GROUP BY characters.species ORDER BY COUNT(species) ASC LIMIT 1
end
